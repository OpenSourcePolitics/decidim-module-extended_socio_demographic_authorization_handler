  ...
  included do
    ...
    def after_confirmation_path_for(resource_name, resource)
      ...
      if first_login_and_not_authorized?(resource)
        decidim_verifications.new_authorization_path(action: :new, handler: :extended_socio_demographic_authorization_handler)
      else
        super
      end
    end
    ...
    def first_login_and_not_authorized?(user)
      user.is_a?(Decidim::User) && user.sign_in_count == 1 && Decidim::Verifications.find_workflow_manifest(:extended_socio_demographic_authorization_handler) && user.verifiable?
    end
  end
end
